...
function credits(project, sources) {
    ...
    for(const source of [...sources].sort((a, b) => a.title.localeCompare(b.title))) {
        const upstream = source.source?.provider === 'modrinth'
            ? ` — Modrinth ${source.source.versionNumber || source.source.projectId}${source.source.projectUrl ? ` (${source.source.projectUrl})` : ''}`
            : ''
        lines.push(`- ${source.title} by ${source.creator} — ${source.license} — revision ${source.revisionId}${upstream}`)
    }
...
function licenseNotice(source) {
    return Buffer.from([
        ...
        ...(source.source?.provider === 'modrinth' ? [
            `Modrinth project: ${source.source.projectId}`,
            `Modrinth version: ${source.source.versionNumber || 'unknown'}`,
            `Modrinth URL: ${source.source.projectUrl || 'unavailable'}`
        ] : []),
        '',
