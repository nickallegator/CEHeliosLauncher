...
function normalizeCategory(value) {
    ...
    if(![CATEGORY_ALL, CATEGORY_SCHEMATICS].includes(normalized)) {
...
function decodeCursor(value, expectedSort) {
    ...
    try {
        ...
        if(decoded.type !== CATEGORY_SCHEMATICS) throw new Error('type')
        if(!/^[a-f0-9-]{36}$/i.test(String(decoded.id || ''))) throw new Error('id')
        return {
...
function normalizeSchematicRow(row) {
    ...
    return {
        ...
        capabilities: { install: true, revision: true, like: true, report: true },
        license: null,
        compatibility: {
            minecraft: '1.21.1',
            loader: 'neoforge',
        ...
        schematic: { version: row.release_version || null, revision }
    }
}

function createCommunityCatalog(options = {}) {
    const database = options.database
    const settings = options.settings || { enabled: false, writeMode: 'disabled' }
    if(!database?.query) throw new TypeError('Community catalog requires a database.')
    ...
    return {
        capabilities() {
            return {
                ...
                categories: settings.enabled === true ? [{
                    id: CATEGORY_SCHEMATICS,
                    readable: true,
                    writable: settings.writeMode !== 'disabled',
                    features: { install: true, revisions: true }
                }] : []
            }
        },
        async list(input = {}) {
            ...
            if(settings.enabled !== true) return { schemaVersion: 1, category, sort, items: [], nextCursor: null }
            ...
            if(cursor) {
                ...
                if(sort === SORT_POPULAR) {
                    where.push(`(coalesce(s.likes, 0) < ${likes}
                        ...
                        or (coalesce(s.likes, 0) = ${likes} and s.updated_at = ${updatedAt} and s.id > ${id}))`)
                } else {
                    where.push(`(s.updated_at < ${updatedAt} or (s.updated_at = ${updatedAt} and s.id > ${id}))`)
                }
            }
            const order = sort === SORT_RECENT
            ...
            const candidates = result.rows.map(normalizeSchematicRow)
