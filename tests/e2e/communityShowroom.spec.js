...
test('local showroom browses and installs every representative content type', async () => {
    ...
    try {
        ...
        const modrinthOrigin = page.locator('#communityModrinthImportOpen')
        await expect(modrinthOrigin).toBeVisible()
        await modrinthOrigin.click()
        const modrinthModal = page.locator('#communityModrinthModal')
        await expect(modrinthModal).toHaveAttribute('data-open', 'true')
        await expect(modrinthModal).toHaveAttribute('aria-hidden', 'false')
        await expect(page.locator('#communityModrinthEmpty')).toBeVisible()
        await expect(page.locator('#communityModrinthEmpty')).toContainText('No Resource Packs are ready to import')
        await expect(page.locator('#communityModrinthEmpty')).toContainText('under review')
        await expect(page.locator('#communityModrinthWorkspace')).toBeHidden()
        await page.keyboard.press('Escape')
        await expect(modrinthModal).toHaveAttribute('aria-hidden', 'true')
        await expect(modrinthOrigin).toBeFocused()

        await page.locator('#settingsMediaButton').click()
        const settingsModrinthConnect = page.locator('#settingsModrinthConnect')
        await expect(settingsModrinthConnect).toBeVisible()
        await settingsModrinthConnect.click()
        await expect(modrinthModal).toHaveAttribute('data-open', 'true')
        await expect(page.locator('#communityModrinthEmpty')).toBeVisible()
        await page.keyboard.press('Escape')
        await expect(modrinthModal).toHaveAttribute('aria-hidden', 'true')
