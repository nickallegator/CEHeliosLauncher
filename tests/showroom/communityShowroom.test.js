...
test('local showroom serves representative read-only artifacts without production services', async t => {
    ...
    assert.equal(distribution.rss, `${runtime.apiBaseUrl}/showroom/news.xml`)
    const newsXml = await fetch(distribution.rss).then(response => response.text())
    assert.equal((newsXml.match(/<item>/g) || []).length, 3)
    assert.match(newsXml, /Community Workshop Opens Its Doors/)
